...
- preconditions
  ...
  - A set of transaction preconditions determining its validity.
    - time_bounds
      - object
      - The time range for which this transaction is valid, with bounds as 64-bit UNIX timestamps
        - min_time
          - string
          - the lower bound
        - max_time
    ...
    - min_account_sequence_age
      - number
      - The minimum duration of time (in seconds) that must have passed since the source account's sequence number changed for the transaction to be valid.
