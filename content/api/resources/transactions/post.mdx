...
This endpoint actually submits a transaction to the Stellar network. It only takes a single, required parameter (the signed transaction). Refer to the [Transactions](../docs/glossary/transactions.mdx) page for details on how to form a proper transaction.
