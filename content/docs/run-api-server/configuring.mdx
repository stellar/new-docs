...
We'll discuss [Ingestion](./ingesting.mdx) more, later.

  - The first parameter, `--ingest`, enables the ingestion subservice.
  - The second parameter, `--captive-core-config-path`, points to a Captive Core configuration file. This file only requires a few fields (explained [below](#configuring-captive-core)) to get up and running. 
...
Some of the underlying Captive Core's functionality should be controlled through this file; it can also be controlled through command-line flags and Horizon provides some defaults, but the TOML file is recommended.


Note that while the Captive Core file looks a lot like a subset of the true Stellar Core configuration file, **this should not be relied upon**. However, you can control some of the underlying Core configuration by setting fields here. Note that Horizon provides defaults for certain fields, others can be specified from the command line, while others still may be changed or introduced in the future. The TOML format is preserved for operator ease of [migrating](./migrating.mdx) from Horizon 1.x, but this is a fundamentally different architecture and should be treated as such.
