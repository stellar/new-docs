...
**Note**: While this may appears to just be a subset of Stellar Core's configuration, you shouldn't think about it that way and treat it as its own configuration. Eventually, all of Horizon will be configured this way.
...
```bash
echo "CAPTIVE_CORE_CONFIG_APPEND_PATH=/etc/default/stellar-captive-core-stub.toml" | sudo tee -a /etc/default/stellar-horizon
```
