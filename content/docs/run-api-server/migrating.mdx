...
The behaviour is slightly different when reingesting old ledgers and when reading recently closed ledgers:
  ...
  - **When reading recently closed ledgers**, Core is started with a normal `run` command. This mode also requires an additional 3GiB of RAM for in-memory ledger entries. In this case, a configuration stub (again, read on [below](#configuration)) **is** required, in order to configure a quorum set so that it can connect to the Stellar network.
