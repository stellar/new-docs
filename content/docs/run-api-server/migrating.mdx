...
```
2021/06/10 13:09:46 Invalid captive core toml file invalid captive core toml: LOG_FILE_PATH in captive core config file does not match Horizon captive-core-log-path flag
```
...
You will need to adjust these accordingly, specifically by pointing `CAPTIVE_CORE_CONFIG_PATH` to your configuration file and possibly `CAPTIVE_CORE_STORAGE_PATH` to where you'd like Captive Core to store its bucket files (keep in mind the [disk space](./prerequisites.mdx) and [permissions](./configuring.mdx#package-manager-installation) requirements).
