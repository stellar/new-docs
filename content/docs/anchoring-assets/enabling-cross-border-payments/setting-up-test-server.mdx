...
**SA**s should poll the `GET /transactions` endpoint(s) provided by their partner **RA**(s) and ensure the transactions initiated are always carried out to completion by fetching updated info from the **SU** when necessary.
...
sequenceDiagram
    ...
    SA Client->>RA Server: [SEP-31 GET /transaction]
    activate RA Server
