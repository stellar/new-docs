 ...
 *     payment operations) of `asset` to withdraw.
 **/
...
```js
/**
 * Submit a payment received from an internal customer.
 *
 * Here, `customerId` is your internal representation of a customer in the
 * pool, passed in to be part of the outgoing transaction's memo purely for
...
Next, if the asset issuer has `authorization_required` set to true, you will need to wait for the trustline to be authorized before you can begin accepting this asset. Read more about [trustline authorization here](https://developers.stellar.org/docs/issuing-assets/control-asset-access/).
