...
The benefit of using a pooled account is *lower costs* – no base reserves needed for each account – and *lower complexity* – you only need to manage one account key. However, with a pooled account it is your responsibility to manage all individual customer balances and payments. You can no longer rely on the Stellar ledger to accumulate value, handle errors and atomicity, and manage transactions on an account-by-account basis.
...
/**
 * Debits a customer (`to`) with `amount` of a particular `asset`.
 *
...
Before we get into the main integration points for adding XLM, make sure to configure your pooled account to require memos. This step is necessary to prevent users from forgetting to attach a memo, which can result in additional support needed from the custodial services and a less desirable user experience for new customers. [This article](https://www.stellar.org/developers-blog/fixing-memo-less-payments) explains how to set that up.

### Listening for incoming payments from the Stellar network
First, you need to listen for payments to the pooled account and debit any user that receives XLM there. For every payment received by the pooled account:
...
Next, if the asset issuer has the `authorization_required` flag on their account set, you will need to wait for them to authorized the trustline before you can begin accepting the asset. Read more about trustline authorization [here](https://developers.stellar.org/docs/issuing-assets/control-asset-access/).

<Alert>

*Note*: Users cannot send a non-native asset to your pooled account (nor receive it from your customers) unless they have opted into that asset by opening a trustline to its issuer.
