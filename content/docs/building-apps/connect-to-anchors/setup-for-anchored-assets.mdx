...
export default async function componentWillLoad() {
  try {
    ...
    this.homeDomain = "<SEP24-ANCHOR-SERVER-DOMAIN>";
...
Once we’ve found an Anchor that supports deposit and withdrawal, we can begin the process of connecting with them from our wallet. 
