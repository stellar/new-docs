...
You can see the data provided by these companies that identifies who they are, what they do, and what services they provide. In this tutorial, we’re interested in the `\[\[CURRENCIES\]\]` they issue as that’s what we’re trying to get ahold of.  We’re also looking for the `TRANSFER\_SERVER` keyword, which indicates that an Anchor supports SEP-24, and the `WEB\_AUTH\_ENDPOINT`, which allows a wallet to set up an authenticated user session. Any time you find these three fields, and you’ve found an Anchor you can interoperate with.
