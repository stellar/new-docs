...
This section of the tutorial takes the [basic wallet](link) with [trustlines](link) and [anchor connections](link) enabled and adds the ability to initiate in-app deposits with an Anchor.  If you have all that ready, and the stellar.toml file is loaded into a `@Prop` on your component, you’re ready to begin the initial deposit of their asset into our wallet. Let’s create `./methods/depositAsset.ts`.
