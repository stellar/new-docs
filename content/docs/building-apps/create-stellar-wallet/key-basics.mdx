...
order: 20
---
> In this tutorial, the goal is to ge to a place where a user can create, store, and access their Stellar account using an intuitive pincode encryption method.  It assumes that you have already completed the [Project Setup](section).
...
## Creating Stellar Account Class
```ts
interface StellarAccount {
  publicKey: string,
  keystore: string,
