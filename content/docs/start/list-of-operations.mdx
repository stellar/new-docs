...
| REVOKE_SPONSORSHIP_MALFORMED | -5 | One or more of the parameters to the operation were invalid. |
...
| LIQUIDITY_POOL_DEPOSIT_UNDERFUNDED | -4 | There's not enough balance of one of the assets to perform the deposit. |
...
| LIQUIDITY_POOL_WITHDRAW_NO_TRUST | -2 | There's no trustline for one of the assets. |
