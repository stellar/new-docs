...
For a description of how operations work in Stellar, see [Operations](link).
...
This can only be called by the issuer of a trustline's [asset](./assets.md), and only when `AUTHORIZATION REQUIRED` (at the minimum) has been set on the issuer's account.
...
The issuer can only change a flag from complete to limited authorization or clear the `authorized` flag if the issuer has the `AUTH_REVOCABLE_FLAG` set. Otherwise, the issuer can only set the `authorized` flag.  For more on what toggling between authorization states allows an issuer to do, see the [Rationale](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0018.md#rationale) section of CAP-18.
