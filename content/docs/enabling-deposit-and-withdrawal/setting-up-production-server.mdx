...
If you're implementing an Issuing Anchor (asset issuer + on/off ramp), make sure you follow [best practices for asset issuance](link) by creating a distribution account that is separate from the issuing account before distributing real assets on the public network. The issuing account secret key should be highly secured via means such as cold storage or multi-signature, and should only be used to send (and, consequently, issue) assets to the distribution account. The distribution account will be used by your server to send (and receive) assets to the users programmatically.
