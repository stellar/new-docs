...
To enable an interactive flow, fiat-backed Anchors (aka on/off ramps) should implement two SEPs in addition to: [SEP-1](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0001.md), which links meta-information to organizations and assets (and which we go over in detail in [this guide](../issuing-assets/publishing-asset-info.mdx)), and [SEP-10](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0010.md), which creates authenticated user session.  In this guide, we'll walk through setting up all that infrastructure on both the testnet and the public network.
