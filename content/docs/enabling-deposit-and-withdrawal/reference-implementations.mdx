...
The SEP-24 Anchor Validation Suite codebase is available in an open-sourced Github Repository, where you can check in details exactly how all tests work. SDF also maintains a deployed version of the Validation Suite with a UI for running the tests without the need to setup your own infrastructure.
In order to also test the interactive parts of SEP-24, you'll need to add test values to all the required interactive flow fields. The Github Repository Readme file has more information on how exactly those fields need to be added.
