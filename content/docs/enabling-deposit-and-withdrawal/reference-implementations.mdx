...
To help with server setup, SDF created [Polaris](link), an extendable django reusable-app that comes with fully-implemented endpoints, templates, and database models. It's built in Python using the community-supported [Stellar Python SDK](link), and is compliant with the Stellar Ecosystem Protocols mentioned in the previous section.
 
In the Polaris repo, you can also find a [Reference Implementation Project](link), a WIP example of an anchor server.
...
Once you have a server set up, SDF also maintains a [Demo Client Project](link) that makes it easy to test your implementation on both the testnet and the pubnet. 

The public [Deployed Demo Client](link) allows you to run those tests without needing to set up a new hosting infrastructure for that project.
...
If you just want to see what the flows look like from a client's point of view, there's also a public [Demo Client Integrated with a Deployed Reference Server](link) that gives a clear step-by-step visual example of how the functionalities work along with insightful information about the requests, protocols, and possible validation messages.  You can run that demo *right now* and to get a sense of how the anchor/wallet interaction works.   
