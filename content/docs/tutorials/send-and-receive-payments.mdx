...
Each of these redeemable assets — *anchored* in the Stellar vernacular — is essentially a credit issued by particular account that represents reserves those accounts hold outside the network.  That's why the assets in the example above had both a `code` and an `issuer`: the `issuer` is the public key of the
account that created the asset, an account owned by the organization that ulitmately honors the credit that asset represents. To find out more about how that works, check out [Enable Deposits and Withdrawals](../enabling-deposit-and-withdrawal/index.mdx). 
