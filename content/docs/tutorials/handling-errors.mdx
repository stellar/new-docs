...
server.submitTransaction(lastTx).catch(function (error) {
  if (isFeeError(error)) {
    let bump = sdk.TransactionBuilder.buildFeeBumpTransaction(
      account, (lastFee * 10).toFixed(2), lastTx, server.networkPassphrase
    );
...
With this trick in our back pocket, resolving 504s is straightforward: after the timebound has expired, try submitting the (*exact same*, incl. the sequence number and signing orders) transaction again with a new timebound. If your transaction made it through and into a ledger at some point after the 504 and before the timebound expired, you'll get your previously-submitted transaction back. If it didn't, you've essentially started from scratch again: you might 504 again, fail for another reason (e.g. `tx_bad_seq`), or succeed.



## Outages
In the case of transaction submission, it's obviously crucial to determine whether or not a transaction went through to the network or not. The best way to do that is simply to retry submission *without modifying the transaction*. Horizon makes an important guarantee (excerpt is from [this example](https://developers.stellar.org/docs/tutorials/send-and-receive-payments/#send-a-payment)):
...
<!-- through querying the `/transactions` endpoint on Horizon. Since the transaction hash is determined *before* it enters the ledger, just checking the path for a 404 after the timebounds tells you that the transaction won't make it into the networks.
 -->
...
function txSubWithBackoff(tx, timeout) {
  return server.submitTransaction(tx).catch(function (error) {
    // transient error? back-off then try again
    if (!error.response) {
