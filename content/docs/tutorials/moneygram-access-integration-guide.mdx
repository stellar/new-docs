...
This document guides the read through the technical requirements for integrating [MoneyGram Access] into an existing application. MoneyGram Access is a MoneyGram product that enables users of third-party applications, such as crypto wallets and exchanges, to cash-in (deposit) and cash-out (withdrawal) of Stellar USDC.

<Alert>


This document does not describe the business requirements for integrating with MoneyGram Access.
...
After Step 4, the application should open the URL provided by MoneyGram in a mobile webview or browser tab and allow the user to complete KYC and transaction initiation with MoneyGram. On completion of this flow, the application’s client should initiate the disbursement of funds.
...
In this section, you will generate at least two Stellar key pairs, one that will be used to prove your application’s identity when authenticating with MoneyGram access, and another that will hold, send, & receive USDC on Stellar.

This section assumes that your application does not have any support for the Stellar network. If your application already supports deposits & withdrawals of XLM, you already have one or more Stellar accounts that can be used for these purposes, although it is heavily encouraged to use a new key pair (that is not funded on Stellar) for authentication.

Go to [Stellar Lab] and generate 2 key pairs. The secret keys should be handled securely, because they will be used to authenticate with and disburse funds to MoneyGram.

The first key pair will be called the “authentication” key pair (or public / secret key). The second key pair will be the “funds” key pair (or account, public key, or secret key).

Provide the public keys (starting with a G) of both the authentication and funds key pairs to MoneyGram. They will add these keys to their known lists of keys, granting them access to their deployment.
...
- The public key of the key pair the application will use to send funds
...
The logic for initiating a deposit transaction looks very similar. See the SEP-24 standard specification for detailed information. MoneyGram requires the `amount` field in both cases.
...
Code for submitting transactions to Stellar should be developed thoughtfully. The SDF has a [documentation page] dedicated to this topic. Here are a few things you need to keep in mind:
...
[stellar ecosystem proposal 24 (sep-24)]: https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0024.md
[sep-24]: https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0024.md
[stellar ecosystem proposal 10 (sep-10)]: https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0010.md
...
[documentation page]: https://developers.stellar.org/docs/tutorials/handling-errors/
