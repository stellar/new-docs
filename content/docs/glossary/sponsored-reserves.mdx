...
```js
  ...
  //
  let s1Account = await server.loadAccount(S1.publicKey())
...
```js
  ...
  //
  let s2Account = await server.loadAccount(S2.publicKey());
...
### Footnote
For the Golang examples, an implementation of `SignAndSend` might look something like this:
