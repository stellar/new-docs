...
First, we'll cover basic liquidity pool participation. <!-- Then, we'll see their seamless integration into path payments. Finally, we'll cover some advanced scenarios with trustline authorization. -->
...
/// Establishes trustlines and pays out to `recipientKp` for all `assets`.
...
Here, we use `distributeAssets()` to establish trustlines and set up initial balances of two custom assets (`A` and `B`, issued by `kp1`) for two accounts (`kp2` and `kp3`).
...
```js
const poolId = sdk.getLiquidityPoolId("constant_product",
  poolShareAsset.getLiquidityPoolParameters()
).toString("hex");

function addLiquidity(account, keypair, keypair, poolId, reserveA, reserveB) {
...
```js
function removeLiquidity(source, sourceKp, poolId, reserveA, reserveB) {
  return server.submitTransaction(
    buildTx(source, sourceKp,
      sdk.Operation.liquidityPoolDeposit({
        liquidityPoolId: poolId,
        minAmountA: reserveA,
        minAmountB: reserveB,
...
function main() {
  return getAccounts()
    ...
    .then(_ => withdrawLiquidiity(accounts[1], kps[1], "500", "2000"))
    .then(_ => getSpotPrice());
}
