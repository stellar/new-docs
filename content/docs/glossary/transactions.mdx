...
For some of these, it's important to understand a related concept: the last time an account touched its sequence number. This occurs both when an account is the source account for a transaction and/or when it is the recipient of a [Bump Sequence](../start/list-of-operations.mdx#bump-sequence) operation. It's worth noting that even if the [Bump Sequence](../start/list-of-operations.mdx#bump-sequence) operation has no effect, i.e. does not change the sequence number, it still counts as a "touch". 

Both the ledger number and timestamp of the last touch is tracked by the network, and these values enable some of the preconditions below (specifically, the minimum sequence [age](minimum-sequence-age) or [ledger gap](minimum-sequence-ledger-gap)).
...
With this precondition, the transaction is only valid after a particular duration (expressed in seconds) elapses since the last time the account's sequence number was touched.

With a minimum [time bound](#time-bounds), the transaction becomes valid after a particular amount of time passes unconditionally. With this precondition, the passage of time applies only to the transaction's source account: you can allow validity after an *account* has waited long enough since its sequence number was touched.

#### Minimum Sequence Ledger Gap
With this precondition, the transaction is only valid after the current network ledger number meets (or exceeds) a particular gap relative to the ledger corresponding to when the source account's sequence number was touched.
...
- **Preconditions** — refer to the [Validity Conditions](#validity-conditions) described above for thorough explanations of each of these. For each of these that are set, the transaction will be considered invalid unless the transaction is submitted:
  ...
  * **Minimum Sequence Age** — refer [above](#minimum-sequence-age) for a thorough explanation
  * **Minimum Sequence Ledger Gap** — refer [above](#minimum-sequence-ledger-gap) for a thorough explanation
