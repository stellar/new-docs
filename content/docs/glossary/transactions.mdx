...
Minimum sequence age is a precondition relating to time, but unlike [time bound](#time-bounds) which is an absolute time, minimum sequence age is relative to the transaction source account's sequence age field.
