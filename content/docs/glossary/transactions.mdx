...
5. **Crafting a candidate transaction set (Validator)**: When it's time to close the ledger, each
   ...
   their fee for inclusion in the set.  See the [Fees](./fees.mdx) for more info.
...
found in [List of Operations](../start/list-of-operationsx.md) doc.
...
| INSUFFICIENT_BALANCE | -7 | Fee would bring account below [minimum reserve](./fminimum-balance.mdx). |
