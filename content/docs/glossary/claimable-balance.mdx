...
async function main() {
  ...
  tx.sign(A);
  return await server.submitTransaction(tx).then(function() {
...
tx = new sdk.TransactionBuilder(aAccount, {fee: sdk.BASE_FEE})
...
tx.sign(A);
return await server.submitTransaction(tx).catch(function (err) {
  console.error(`Tx submission failed: ${err}`)
});
```
