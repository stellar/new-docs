...
```js
  ...
  //
  // Method 2: Suppose `txResponse` comes from a continuation of the
  // transaction submission Promise chain above.
  ...
  .then(async function(balanceId) {
    let claimBalance = sdk.Operation.claimClaimableBalance({ balanceId: balanceId })

    let aAccount = server.loadAccount(A.publicKey())
    let tx = new sdk.TransactionBuilder(aAccount, {fee: sdk.BASE_FEE})
    ...
    // Note that this will fail until a minute elapses:
