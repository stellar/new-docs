...
var A = sdk.Keypair.fromSecret("SBPQUZ6G4FZNWFHKUWC5BEYWF6R52E3SEP7R3GWYSM2XTKGF5LNTWW4R")
var B = sdk.Keypair.fromPublicKey("GAS4V4O2B7DW5T7IQRPEEVCRXMDZESKISR7DVIGKZQYYV3OSQ5SH5LVP")

// NOTE: Error checks are omitted for brevity; always validate things!

server
  .loadAccount(A.publicKey())
  .then(function(aAccount) {
    // Create a claimable balance with our two above-described conditions.
    let soon = Date.now() + 60
    ...
    let tx = new sdk.TransactionBuilder(aAccount, {
    ...
    tx.sign(A)
    return [tx, server.submitTransaction(tx)]
  })
  // Method 1: Suppose `tx` comes from the transaction built above.
  //           Notice that this can be done *before* submission.
  // 
  // TODO: Before this can be merged, the tx.claimableBalanceId(int) helper
  //       actually needs to be implemented lol
  .then(function(pair) {
    // why do people put up with a language that doesn't have multiple
    // return values in 2020 lmao
    let tx = pair[0], txResponse = pair[1]
    console.log("Claimable balance created!")

    // let balanceId = tx.ClaimableBalanceID(0)
  ...
  .then(function(txResponse) {
    let txResult = sdk.xdr.TransactionResult.fromXDR(txResponse.result_xdr, "base64")
  ...
  .then(function(balanceId) {
    console.log("Claiming", balanceId, "as", A.publicKey())
    let claimBalance = sdk.Operation.claimClaimableBalance({
      balanceId: balanceId
    })

    let tx = new sdk.TransactionBuilder(aAccount, // or Account B!
      {
        networkPassphrase: sdk.Networks.TESTNET,
        fee: sdk.BASE_FEE,
      })
