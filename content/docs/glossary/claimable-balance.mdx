...
async function main() {
  ...
  var A = sdk.Keypair.fromSecret("SAQLZCQA6AYUXK6JSKVPJ2MZ5K5IIABJOEQIG4RVBHX4PG2KMRKWXCHJ");
  ...
  // Ideally, we should also ensure the destination account(s) exist so any
  // claimable funds don't get lost to oblivion, but since we're including a
  // "owner can reclaim" predicate (see below), we don't necessarily need this
  // sanity check.
  // let bAccount = await server.loadAccount(B.publicKey())
...
```js
async function main(txResponse) {
  // Method 1: Not available in the JavaScript SDK yet.


  // Method 2: Suppose `txResponse` comes from the `submitTransaction()` above.
