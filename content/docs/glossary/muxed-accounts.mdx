...
**Warning**: _This feature is experimental._ Muxed accounts are not widely supported yet. Many wallets may not display a muxed account address and instead display the account ID address, ignoring the 64-bit ID. Adoption efforts are actively in progress.
...
Both of these addresses, when used with one of the [supported operations](muxed-accounts.mdx#supported-operations), will act on the underlying `GA7Q...` address.
...
It's worth noting that this feature is intended to be a high-level abstraction, merely embedded into the protocol for convenience and standardization. There's no validation on IDs: as far as the Stellar Network is concerned, all of the [supported operations](muxed-accounts.mdx#supported-operations) operate exactly as if you did _not_ used a muxed account. For example, if you make two payments from two muxed accounts that share an underlying Stellar account (i.e. muxed accounts with the same underlying `G...` account but two different IDs), this is _exactly the same_ as that single Stellar account sending two payments, as far as the ledger is concerned.

Even though only the underlying `G...` account _truly_ exists in the Stellar ledger, the [Horizon API](broken-reference) will make some effort to interpret and track the muxed accounts responsible for certain actions.
...
We'll demonstrate some of these in the [Examples](muxed-accounts.mdx#examples).
...
* [normal](muxed-accounts.mdx#payments), "full" Stellar account payments (i.e. G to G),
* [mixed](muxed-accounts.mdx#muxed-to-unmuxed) payments (i.e. M to G), and
* [fully muxed](muxed-accounts.mdx#muxed-to-muxed) payments (i.e. M to M)
...
As is the case for most protocol-level features, you can find more usage examples and inspiration in the relevant test suite for your favorite SDK. For example, [here](https://github.com/stellar/js-stellar-base/blob/master/test/unit/muxed\_account\_test.js) are some of the JavaScript test cases.
