...
A fee-bump transaction is essentially a wrapper around a transaction that has been bundled with requisite signatures into a transaction envelope.  Therefore, before creating a fee-bump transaction, you must first create a regular transaction and transacrtion envelope.  For more on regular transaction creation and a summery of the lifecycle of a transaction, see [Transactions](/transactions.mdx).   
