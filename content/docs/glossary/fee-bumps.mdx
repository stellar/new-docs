...
Once a fee-bump transaction is ready to be signed, it's wrapped in a transaction envelope, which contains the fee-bump transaction as well as the signature of the fee account specified in the fee-bump transaction.  Fee-bump transaction envelopes behave just like regular [transaction envelopes](./transactions.mdx/#transaction-envelopes). 
...
- **Inner Transaction Validity** - The inner transaction must be valid.  If validation of the inner transaction is successful, then the result is `FEE_BUMP_INNER_SUCCESS`, and the validation results from the validation of the inner transaction appear in the inner result.  If the inner transaction is invalid, the result is `FEE_BUMP_INNER_FAILED`, and the fee-bump transaction is invalid because the inner transaction is invalid.  For more on the validity of regular transactions, see [Validity of a Transaction](./transactions.mdx/#validity-of-a-transaction)  
