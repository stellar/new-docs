...
A fee-bump transaction is essentially a wrapper around a transaction that has been bundled with requisite signatures into a transaction envelope.  Therefore, before creating a fee-bump transaction, you must first create a regular transaction and transacrtion envelope.  For more on regular transaction creation, see [Transactions](/transactions.mdx).   

1. **Creation (Fee-bump Transaction Creator)**: A user creates fee-bump transaction by taking an existing transaction envelope and specifying a fee account and maximum fee amount.  
   
2. **Signing (Transaction Signers)**: The user forms a fee-bump transaction envelope, which contains the fee-bump transaction along with the signature of the fee account.
...
4. **Propagating (Validator)**: Once Stellar Core has determined that a fee-bump transaction is valid, it
   ...
   way, a valid fee-bump transaction is flooded to the entire Stellar network.
