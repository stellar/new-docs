...
## Validity of Inner Transaction
For a fee bump to succeed, the inner transaction must be valid.  If validation of the inner transaction is successful, then the result is `FEE_BUMP_INNER_SUCCESS`, and the validation results from the validation of the inner transaction appear in the inner result.  If the inner transaction is invalid, the result is `FEE_BUMP_INNER_FAILED`, and the fee-bump transaction is invalid because the inner transaction is invalid.  For more on the validity of regular transactions, see [Validity of a Transaction](./transactions.mdx/#validity-of-a-transaction) 
