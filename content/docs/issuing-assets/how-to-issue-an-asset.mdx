...
Distributing assets through a distribution account is a design pattern. Functionally, you can do away with the distribution account and distribute directly from the issuer account. A less know fact is that you can even create a market directly from the issuing account and issue by trading.
...
The account you use to distribute your asset from is going to be a _hot_ account, meaning that some web service out there has direct access to sign it's transactions.
...
The second reason is book keeping or auditability. The issuer account can't actually hold a balance of their own asset. If you have standing inventory of your own asset in a seprate account it is easier to track. This is a common pattern in various ledgering solutions.
