---
title: Publishing information about an asset
order: 40
---
...
Using a `set_options` operation, you can link your Stellar account to the domain that hosts your `stellar.toml`, thereby creating a definitive on-chain link between this information and that account.

## How to complete your stellar.toml

The very first Stellar Ecosystem Proposal, aptly named SEP-0001, specifies everything you could ever want to include in a `stellar.toml` file.  This guide, which is targeted toward asset issuers, won’t cover the `Validator Information` section (that’s covered elsewhere), and may omit some details relevant to your use case.  

The goal here is to walk through the sections of SEP-0001 that directly relate to asset issuers, so you should use this guide in conjunction with that SEP to make sure you complete your `stellar.toml` correctly.  The four sections we’ll cover:
