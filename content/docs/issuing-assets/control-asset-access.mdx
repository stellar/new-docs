...
There are three possible flags, and they are created on the _account level_ using a `set_options` operation. They can be set at any time in the life cycle of an asset, not just when you issue it:
