...
- too many validators missing (down or without a good connectivity), solutions are:
  - [adjust your quorum set](./configuration.mdx#choosing-a-quorum-set) based on the nodes that are not missing
  - try to get a [better connectivity path](./configuration.mdx#quorum-and-overlay-network) to the missing validators
