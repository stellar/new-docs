...
The below example is for [/glossary](https://github.com/stellar/new-docs/blob/master/content/docs/glossary/metadata.json) page
...
Currently (April, 2020), only [Documentation](https://developers.stellar.org/docs/) uses images.
...
* **H2** tags populate the ride-side page [table of contents](https://developers.stellar.org/docs/anchoring-assets/enabling-deposit-and-withdrawal/setting-up-test-server/) in Documentation
...
For example, [Api Reference > Resources > Operations > Object](https://developers.stellar.org/api/resources/operations/object/)
...
Example: `<AttributeTable/>` without type specified on [API's Manage Data Object](https://developers.stellar.org/api/resources/operations/object/manage-data/).

![AttributeTable with Type](readme-imgs/attributetable-with-type.png)

Example: `<AttributeTable/>` with type specified on [API's Effect Types](https://developers.stellar.org/api/resources/effects/types/).

![AttributeTable without Type](readme-imgs/attributetable-without-type.png)
...
For example, [Setting Up Test Server](https://developers.stellar.org/docs/anchoring-assets/enabling-deposit-and-withdrawal/setting-up-test-server/)
...
`<CodeExample/>` is a code snippet component. You can include snippets for more than one language. See an example of including a snippet for `curl` and `JavaScript` below. It is using [Prism syntax highlighting library](https://prismjs.com).
...
For example, [Setting Up Test Server](https://developers.stellar.org/docs/anchoring-assets/enabling-deposit-and-withdrawal/setting-up-test-server/) in Documentation and [Resources > Transaction > Retrieve a Transaction](https://developers.stellar.org/api/resources/transactions/single/) in API Reference.
...
If you would like to add an additional language to this, visit [CodeExample.js](https://github.com/stellar/new-docs/blob/master/src/components/CodeExample.js) and add it to `CODE_LANGS`.
...
For example, [Aggregations > Order Books > Retrieve an Order Book](https://developers.stellar.org/api/aggregations/order-books/single/).
...
`<EndpointsTable/>` displays an endpoint and its [HTTP method](https://en.wikipedia.org/wiki/Hypertext\_Transfer\_Protocol#Request\_methods).

* PropTypes
  * `title` (required)
  * `children` (required)

For example, [Resources > Transactions](https://developers.stellar.org/api/resources/transactions/).
...
For example, [Introduction > XDR](https://developers.stellar.org/api/introduction/xdr/).
...
![MethodTable Component](readme-imgs/MethodTable.png)
...
Chrome allows you to [override the user agent](https://developers.google.com/web/tools/chrome-devtools/device-mode/override-user-agent) with DevTools, which enables us to test how Google will see pages.
